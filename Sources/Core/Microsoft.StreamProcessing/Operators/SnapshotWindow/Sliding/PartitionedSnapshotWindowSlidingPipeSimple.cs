...
namespace Microsoft.StreamProcessing
{
    ...
    /// </summary>
    [DataContract]
    internal sealed class PartitionedSnapshotWindowSlidingPipeSimple<TInput, TState, TOutput, TPartitionKey> : UnaryPipe<PartitionKey<TPartitionKey>, TInput, TOutput>
    {
        ...
        public PartitionedSnapshotWindowSlidingPipeSimple() { }
        ...
        private void AdvanceTime(PartitionEntry partition, long syncTime)
        {
            ...
            {
                var ecqState = partition.ecq.Dequeue();
                if (partition.currentState.active > 0)
                {
                    // Issue end edge
                    int c = this.batch.Count;
