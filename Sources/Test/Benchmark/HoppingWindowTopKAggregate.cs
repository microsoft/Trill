...
namespace Benchmark
{
    public class HoppingWindowTopKAggregateBenchmark
    {
        ...
        [Benchmark]
        public void TestHoppingWindowTopKAggregate()
        {
            HoppingWindowTopKAggregateRandomDistribution();

            const int K = 3;

            var output = data.ToStreamable().HoppingWindowLifetime(WindowSize.Ticks, HopSize.Ticks).TopK(K);
