...
namespace ComponentTesting.Collections
{
    ...
    public class EndPointQueueTest : TestWithConfigSettingsAndMemoryLeakDetection
    {
        ...
        private static void SimpleEndPointQueueTest()
        {
            ...
            Assert.IsFalse(queue.TryPeekNext(out long time, out int value));
            Assert.IsFalse(queue.TryGetNext(out time, out value));
            Assert.IsFalse(queue.TryGetNextExclusive(100, out time, out value));
            Assert.IsFalse(queue.TryGetNextInclusive(100, out time, out value));

            queue.Insert(5, 1000);

            Assert.IsFalse(queue.IsEmpty);
            Assert.AreEqual(1, queue.Count);
            Assert.IsFalse(queue.TryGetNextExclusive(1, out time, out value));
            Assert.IsFalse(queue.TryGetNextInclusive(1, out time, out value));
            Assert.IsTrue(queue.TryPeekNext(out time, out value));
            ...
            Assert.IsFalse(queue.TryGetNextInclusive(2, out time, out value));
            Assert.IsTrue(queue.TryGetNextInclusive(3, out time, out value));
            Assert.AreEqual(3, time);
            Assert.AreEqual(1001, value);

            Assert.IsFalse(queue.TryGetNextExclusive(4, out time, out value));
